...
class BaseTableViewTest(unittest.TestCase):
    ...
    def test_list_basetable(self):
        url = reverse('Forecasting_basetable_list')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_create_basetable(self):
        url = reverse('Forecasting_basetable_create')
        data = {
            "name": "name",
    ...
    def test_detail_basetable(self):
        ...
        url = reverse('Forecasting_basetable_detail', args=[basetable.slug,])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_basetable(self):
        ...
        url = reverse('Forecasting_basetable_update', args=[basetable.slug,])
